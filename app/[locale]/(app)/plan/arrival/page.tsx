...
export default async function ArrivalPage({ params }: PageProps<"/[locale]/plan/arrival">) {
  ...
  const [applications, { data: tasks }] = await Promise.all([
    ...
    supabase.from("tasks").select("id, application_id, title").eq("user_id", user.id).eq("task_type", "enrollment"),
  ]);
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader title={t("heading")} description={t("subheading")} />
      <p className="rounded-md bg-amber-50 px-4 py-3 text-sm text-amber-800">{t("varyByCountryNote")}</p>
